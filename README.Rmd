...
- 1.`data/`: Data folder\
  ...
  - vi.`RF_Harvest_Reconstruction/`: Statewide rockfish harvest and release estimates using the Howard et al. method. This data is used for the annual rockfish harvest reconstruction.\       
    * This data comes from Phil Joy, and can be found at <https://github.com/ADFG-DSF/RockfishSportMort/output/>\
...
- 4.`scripts/`: Scripts for running analyses\
  ...
  - iii.`L.WRegression/`: Calculate yield of groundfish in kg, using length-weight regressions to estimate weights of fish when no weight is available in the data.\ 
...
Estimates are preliminary because they are based on L-W regressions that haven't yet been tested for differences between years, ports, sexes, etc.\
  ...
  - iv. `Objectives`: Analyzing primary and secondary objectives of the port sampling program\
    * a. `Objectives 1 & 2`: Estimate mean weight and length of Pacific halibut by user group and sport fish area\
    * b. `Objective 3`: Estimate species compe of rockfish by port\
    * c. `Objective 4`: Estimate age, length, and sex composition of rockfish by port for principal rockfish species (black/yelloweye)\
    * d. `Objective 5`: EStimate age, length, and sex composition of lingcod by port\
    * e. `Secondary Objective 1`: Identify differences in geographic distribution of groundfish harvest and effort by user group and year\
    * f. `Secondary Objective 2`: Estimate proportion of Pacific halibut harvest cleaned at sea in each port\
    * g. `Secondary Objective 3`: Estimate proportion of Pacific halibut caught on circle hooks vs. other hook types at each port\
    * h. `Secondary Objective 4`: Evaluated data on discarded rockfish depth of capture for pelagic, yelloweye, and non-pelagic assemblages\
    * i. `Secondary Objective 5`: Estimate proportions of released lingcod of sublegal size by port\
  ...
  - x. `RFLW.R`: Estimates length- weight parameters for rockfish using a log-log model of logWeight estimated from logLength. The predicted weight parameters produced by this file are used to derive predicted weight for rockfish with no weight estimate in the rockfish harvest reconstruction file and elsewhere.
