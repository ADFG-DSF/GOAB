################################################################################
# Apportion halibut to mgmtareas.sas
# 
...
#Estimate MgmtArea of capture proportions for halibut harvest by port using interview data
intnoNA <- int %>%
  filter(
         HAKEPT > 0,   # Remove trips with no halibut harvest
         ADFGSTAT != 'Err'   # Remove trips with errors in stat area
         )
...
gt(
  ...
  reframe(MgmtArea = unique(MgmtArea), w_hakept = sum(HAKEPT)) %>% # calculate sum of hakept for each unique combination of port, user, year, and MgmtArea
  # ungroup() %>% # remove grouping
  group_by(PORT, MgmtArea) %>% # group by port and MgmtArea
  reframe(prop = sum(w_hakept) / sum(intnoNA$HAKEPT)) # calculate the proportion of hakept for each MgmtArea and port, using the total sum of hakept in the dataset
)
...
#Estimate proportions of Seward halibut harvest by MgmtArea
# sort the data
awl_sorted <- awl %>%
